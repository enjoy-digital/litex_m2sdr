...
int main(int argc, char **argv)
{
    ...
    if (do_stream_loopback) {
        ...
        if (rc == M2SDR_ERR_UNSUPPORTED) {
            ...
            int16_t *tx_buf = m2sdr_alloc_buffer(format, samples_per_buf);
            int16_t *rx_buf = m2sdr_alloc_buffer(format, samples_per_buf);

            if (!tx_buf || !rx_buf) {
                print_status("DMA streaming loopback buffers", M2SDR_ERR_NO_MEM, &errors);
                m2sdr_free_buffer(tx_buf);
                m2sdr_free_buffer(rx_buf);
                m2sdr_set_dma_loopback(dev, false);
                m2sdr_close(dev);
                return 1;
            }

            for (unsigned i = 0; i < (m2sdr_samples_to_bytes(format, samples_per_buf) / sizeof(int16_t)); i++)
