 ...
 * the capability register is what distinguishes the images. */
static bool m2sdr_has_rfic_oversampling(struct m2sdr_dev *dev)
{
                                 ...
                                 unsigned rotate)
{
     ...
     *
     * The pair-mismatch metric is blind to some whole-UI capture shifts (both
     * channels shift identically, so ia==ib still holds while the words are
     * wrong), so the first clock delay with clean per-lane windows can still
     * fail the caller's sequence-level PRBS verify. `rotate` starts the
     * candidate scan at a different clock delay so the caller can explore all
     * of them across its verify retries; a clock delay that already passed
     * the verify (dev->rf_deskew_clk) is preferred when rotate == 0. */
    ...
    for (ci = 0; ci < sizeof(clk_delays); ci++) {
        unsigned center[6];
        bool all_found = true;
        uint8_t clk_delay = clk_delays[(ci + rotate) % sizeof(clk_delays)];

        if (rotate == 0 && dev->rf_deskew_clk != 0) {
            /* Keep the verified-good clock delay first; fill the rest in list order. */
            if (ci == 0) {
                clk_delay = dev->rf_deskew_clk;
            } else {
                unsigned k, n = 0;
                clk_delay = clk_delays[0];
                for (k = 0; k < sizeof(clk_delays); k++) {
                    if (clk_delays[k] == dev->rf_deskew_clk)
                        continue;
                    if (++n == ci) {
                        clk_delay = clk_delays[k];
                        break;
                    }
                }
            }
        ...
        ad9361_spi_write(phy->spi, REG_RX_CLOCK_DATA_DELAY, clk_delay << 4);
        mdelay(2);
        ...
        if (getenv("M2SDR_DESKEW_DEBUG")) {
            for (lane = 0; lane < 6; lane++) {
                char line[512];
                int  n = snprintf(line, sizeof(line), "deskew dbg clk %u lane %u:", clk_delay, lane);
                for (tap = 0; tap < 32; tap++)
                    n += snprintf(line + n, sizeof(line) - (size_t)n, " %u",
                                  errs[lane][tap] > 999999u ? 999999u : errs[lane][tap]);
                M2SDR_LOGF("%s\n", line);
            }
        }

        for (lane = 0; lane < 6; lane++) {
            /* Pick each lane's FIRST adequate window, not its longest: only the
             * first eye is usable (see above). A lane whose first eye is partly
             * clipped below tap 0 can present a LONGER second eye one UI later,
             * which the pair-mismatch metric cannot reject (with the same BIST
             * word in every slot a whole-UI-shifted lane still compares equal)
             * but which scrambles the assembled words - the sequence-level
             * verify then fails at every clock delay. A minimum window length
             * keeps single-tap noise flukes from masquerading as the eye. */
            const unsigned min_eye = 3;
            unsigned best_start = 0, best_run = 0, start = 0, run = 0;
            unsigned first_start = 0, first_run = 0;
            ...
            for (tap = 0; tap < 32; tap++) {
                if (errs[lane][tap] == 0) {
                    ...
                    if (first_run == 0 && run >= min_eye) {
                        first_start = start;
                        first_run = run; /* grown below while the run continues */
                    } else if (first_run != 0 && start == first_start) {
                        first_run = run;
                    }
                } else {
                    run = 0;
            ...
            M2SDR_LOGF("RX deskew: clk %u lane %u window [%u..%u] -> tap %u.\n",
                clk_delay, lane, best_start, best_start + best_run - 1, center[lane]);
        }
        if (!all_found) {
        ...
        dev->rf_deskew_clk = clk_delay; /* tentative until the caller's verify passes */
...
#ifdef CSR_AD9361_PHY_CONTROL_RX_FRAME_OFFSET_OFFSET
/* Rotate the RX slot decode vs the RX_FRAME edge (2R2T, see the gateware field). */
static void m2sdr_set_rx_frame_offset(struct m2sdr_dev *dev, unsigned off)
{
    uint32_t v = 0;

    (void)m2sdr_reg_read(dev, CSR_AD9361_PHY_CONTROL_ADDR, &v);
    v &= ~(0x3u << CSR_AD9361_PHY_CONTROL_RX_FRAME_OFFSET_OFFSET);
    v |= (off & 0x3u) << CSR_AD9361_PHY_CONTROL_RX_FRAME_OFFSET_OFFSET;
    (void)m2sdr_reg_write(dev, CSR_AD9361_PHY_CONTROL_ADDR, v);
}
#endif
                                        ...
                                        uint32_t ad9361_rate)
{
    ...
    for (try_ = 1; try_ <= 20; try_++) {
        ...
        if (m2sdr_has_rfic_oversampling(dev)) {
            ...
            if (!(phy_control & (1u << CSR_AD9361_PHY_CONTROL_MODE_OFFSET))) {
                /* Start each verify retry at a different clock-delay candidate:
                 * the pair metric alone can accept a whole-UI-shifted capture
                 * that only the sequence verify catches (board-dependent). */
                (void)m2sdr_deskew_rx_lanes(dev, phy, (unsigned)(try_ - 1));
            } else {
                unsigned lane;
...
#ifdef CSR_AD9361_PHY_CONTROL_RX_FRAME_OFFSET_OFFSET
        /* RX_FRAME slot rotation: the frame lane has no per-lane delay, so its
         * board skew can shift the slot decode by whole DATA_CLK cycles - a
         * rotation the deskew's pair metric cannot see (identical BIST words
         * in every slot) but the sequence verify catches. The rotation is
         * independent of the lane taps, so probe all four on one deskew,
         * starting from the last verified value. */
        {
            uint32_t phy_control = 0;
            unsigned off_i;
            ...
            if (!(phy_control & (1u << CSR_AD9361_PHY_CONTROL_MODE_OFFSET))) {
                rc = M2SDR_ERR_IO;
                for (off_i = 0; off_i < 4 && rc != M2SDR_ERR_OK; off_i++) {
                    unsigned off = (dev->rf_rx_frame_offset + off_i) & 0x3u;

                    m2sdr_set_rx_frame_offset(dev, off);
                    rc = m2sdr_wide_bandwidth_verify(dev);
                    if (rc == M2SDR_ERR_OK && off != 0)
                        M2SDR_LOGF("RX frame-slot rotation %u verified.\n", off);
                    if (rc == M2SDR_ERR_OK)
                        dev->rf_rx_frame_offset = (uint8_t)off;
                }
            } else {
                m2sdr_set_rx_frame_offset(dev, 0);
                rc = m2sdr_wide_bandwidth_verify(dev);
            }
        }
#else
                                    ...
                                    enum m2sdr_channel_layout channel_layout)
{
    ...
    if (cfg->sample_rate > 61440000) {
         ...
         * per-lane taps left by an earlier wide-mode configuration. */
        unsigned lane;
...
int m2sdr_apply_config(struct m2sdr_dev *dev, const struct m2sdr_config *cfg)
{
     ...
     * in-place clock re-program does not). Bounded to 12 re-rolls; a standard image never reaches
     * the framing check (the rate/layout is rejected above), so it configures once. */
    for (unsigned fr_attempts = 0; ; fr_attempts++) {
        ...
        if (cfg->sample_rate > 61440000 && channel_layout == M2SDR_CHANNEL_LAYOUT_2T2R) {
            ...
            (void)m2sdr_deskew_rx_lanes(dev, phy, 0); /* reuses the verified clock delay */
        }
        break;
    }
