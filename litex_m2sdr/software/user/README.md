...
- **libm2sdr/**: Board-specific support for the M2SDR, integrating device configuration (clocking, RFIC backend selection, etc.) and exposing higher-level APIs to these utilities.
...
- `--refclk-freq hz` RFIC reference clock generated by SI5351 (typically `38400000` or `40000000`)
  With `--sync external`, the SI5351 input is an external `10MHz` CLKIN (UFL), but `--refclk-freq` remains the generated RFIC reference clock.
