...
class PTPTimeDiscipline(LiteXModule):
    ...
    def __init__(self, sys_clk_freq, nominal_time_inc, with_csr=True):
        ...
        self.phase_lock_window_cfg = Signal(32, reset=500)
        ...
        self.comb += [
            ...
            If(~self.enable,
                next_locked.eq(0),
            ).Elif(~self.ptp_locked,
                next_locked.eq(0),
            ).Elif((error_abs >= self.coarse_threshold_cfg) | ~ever_locked,
                next_locked.eq(0),
            ).Else(
                next_locked.eq(error_abs <= self.phase_lock_window_cfg),
            ),
        ]
        ...
        self.sync += [
            If(self.clear_counters,
                ...
                If(prev_ptp_locked & ~self.ptp_locked,
                ...
                prev_ptp_locked.eq(self.ptp_locked),
                ...
                If(self.ptp_locked,
                    self.holdover.eq(0),
                    If((error_abs >= self.coarse_threshold_cfg) | ~ever_locked,
                        ...
                        If(error_abs >= self.phase_threshold_cfg,
                        ...
                        If((integral_trim + error_ns) < -self.trim_limit_cfg,
                            integral_trim.eq(-self.trim_limit_cfg),
                        ).Elif((integral_trim + error_ns) > self.trim_limit_cfg,
                            integral_trim.eq(self.trim_limit_cfg),
                        ).Else(
    ...
    def add_csr(self):
        ...
        self._update_cycles = CSRStorage(32, reset=self.update_cycles_cfg.reset.value, description="Servo update interval in sys_clk cycles.")
        self._coarse_threshold = CSRStorage(32, reset=self.coarse_threshold_cfg.reset.value, description="Coarse realignment threshold in nanoseconds.")
        self._phase_threshold = CSRStorage(32, reset=self.phase_threshold_cfg.reset.value, description="Bounded phase-trim threshold in nanoseconds.")
        self._lock_window = CSRStorage(32, reset=self.phase_lock_window_cfg.reset.value, description="Time-lock window in nanoseconds.")
        self._phase_step_shift = CSRStorage(6, reset=self.phase_step_shift_cfg.reset.value, description="Right shift applied to the absolute phase error before bounded trims.")
        self._phase_step_max = CSRStorage(32, reset=self.phase_step_max_cfg.reset.value, description="Maximum bounded phase trim in nanoseconds.")
        self._trim_limit = CSRStorage(32, reset=self.trim_limit_cfg.reset.value, description="Maximum frequency trim in time_inc LSB units.")
        self._p_gain = CSRStorage(16, reset=self.p_gain_cfg.reset.value, description="Proportional gain applied to the phase error before rate trim limiting.")
        ...
        self._ptp_lock_losses = CSRStatus(32, description="Number of PTP lock-loss events observed by the discipline loop.")
        self._time_lock_losses = CSRStatus(32, description="Number of board-time lock-loss events observed by the discipline loop.")
