...
class AsyncFIFORegistered(Module, _FIFOInterface):
    ...
    def __init__(self, width, depth, register_storage=False, registered_read=False,
        ...
        assert not (registered_read and registered_write)
        ...
        if registered_read:
            ...
            # Storage never needs a reset (its contents are qualified by the pointers); reset_less
            # keeps the domain reset off the width*depth FF-array R pins - a fanout that cannot
            # close timing in a 491.52MHz write domain.
            words = Array(Signal(width, reset_less=True) for _ in range(depth))
            ...
            words = Array(Signal(width, reset_less=True) for _ in range(depth))
            if registered_write:
                # Fanout pipeline for a write domain at the rfic_clk Oversampling rate: din and the
                # one-hot row select are registered first, so the wide nets become local per-row
                # registered drivers (din -> din_r is a 1:1 capture; each ce_r bit drives only its
                # own row's clock enables and can be placed beside it). The row content lands one
                # write cycle after the pointer advance - safe as long as the read period is not
                # shorter than the write period (the pointer needs >= 2 read-domain synchronizer
                # cycles to become observable, by which time the content has long landed).
                din_r = Signal(width, reset_less=True)
                ce_r  = Signal(depth)
                self.sync.write += [
                    din_r.eq(self.din),
                    ce_r.eq(Mux(produce.ce, 1 << produce.q_binary[:-1], 0)),
                ]
                self.sync.write += [If(ce_r[i], words[i].eq(din_r)) for i in range(depth)]
            else:
                self.sync.write += If(produce.ce, words[produce.q_binary[:-1]].eq(self.din))
            dout = Signal(width)
