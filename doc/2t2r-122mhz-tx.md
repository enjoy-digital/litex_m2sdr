...
In `--with-rfic-oversampling` builds the six TX data lanes and TX_FRAME are driven by
**OSERDESE2** (8:1 DDR) instead of ODDRs:
...
- The sample groups cross from the `rfic` domain (491.52 MHz, BUFG of `DATA_CLK`) into the
  `CLKDIV` domain through a small gray-pointer async FIFO. The two domains' relative phase is
  arbitrary (it follows the MMCM lock and the free-running group counter), so a plain
  re-registration can tear groups on some lock alignments; the FIFO handshake makes the
  transfer phase-agnostic so every timing-met build and every MMCM lock transfers whole groups.
- All seven OSERDESE2 (6 data lanes + TX_FRAME) share one CLKDIV-synchronous reset. The
  serializer's internal 8:1 load phase is established at reset release, so every instance must
  leave reset in the same CLKDIV cycle: with a raw async reset an instance can come up
  serialized a whole bit-cycle offset from the others (a per-build routing lottery that shows
  as one lane with a deterministic 100 % error rate no delay tap can fix).
...
```sh
# Gateware (oversampling build, OSERDESE2 TX serializer):
python litex_m2sdr.py --variant=m2 --with-pcie --pcie-gen=2 \
