...
Do not edit generated software headers directly. `litex_m2sdr/software/kernel/csr.h`,
`mem.h` and `soc.h` are generated by `scripts/gen_kernel_headers.py` from *every* supported
build configuration at once (`litex_m2sdr/build_configs.py`), not from one build: each
configuration is required to place every CSR it shares with the others at the same address, so
one build of the host software drives any image. Re-run that script after changing the CSR map;
`test/test_csr_layout.py` fails if the configurations disagree or the headers are stale. A build
writes its own headers to `build/<build_name>/kernel/` and leaves the tracked ones alone.

Because of this, an address is never image-specific: a feature that only some images implement is
detected at runtime through the capability register (`m2sdr_get_capabilities`), never with
`#ifdef CSR_...`, which is now either always true or always false.
