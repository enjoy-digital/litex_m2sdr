...
| PCIe | raw DMA benchmark, 16 MiB | 93.202 MiB/s | 63.391 MiB/s | `diag pcie-dma-bench` write/read/verify passed |
...
| Ethernet/Etherbone | raw `diag write`/`diag read`, 16 MiB | 46.0, 53.5, 53.5 MiB/s | 31.5, 34.0, 34.0 MiB/s | all readbacks matched with `cmp` |
...
`diag write`/`diag read` moves only a raw sector payload to/from a fixed sector
range and is the clearest host-to-SATA/SATA-to-host payload throughput number.
`import`/`export` measures the end-to-end named-capture workflow:
...
./m2sdr_sata -c 0 --pattern counter diag pattern-write 0x240000 8192
./m2sdr_sata -c 0 --pattern counter diag pattern-check 0x240000 8192
```
...
./m2sdr_sata -i 192.168.1.50 diag etherbone-bench --iterations 3
...
4 MiB of counter data. SigMF round-trips used `import` and `export`;
...
`diag read 0x128000 32768` transfer over Ethernet + SATA:
...
- If `diag pcie-dma-bench` reports `unsupported`, the loaded `m2sdr.ko` does not
