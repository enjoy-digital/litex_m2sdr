...
#include "m2sdr.h"
#include "csr.h"
#include "config.h"   /* DMA_BUFFER_COUNT: TX DMA ring depth (sets the timed-TX pipeline latency) */
...
int main(int argc, char **argv)
{
    ...
    int    chan = 2, reps = 16, layout2 = 1; long set_off = -1;
    /* Defaults suit a bare TX->RX cable; a conducted loopback with fixed attenuators needs the
     * link budget compensated (e.g. a 40 dB pad: --rx-gain 54 --tx-att 0), otherwise the marker
     * lands in the noise and no air-time can be measured. */
    int    rx_gain = 40, tx_att = 10;
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--rx-gain")    && i + 1 < argc) rx_gain = atoi(argv[++i]);
        else if (!strcmp(argv[i], "--tx-att")     && i + 1 < argc) tx_att  = atoi(argv[++i]);
        else if (!strcmp(argv[i], "--layout")     && i + 1 < argc) layout2 = !strcmp(argv[++i], "2t2r");
        else { fprintf(stderr, "unknown arg %s\n", argv[i]); return 2; }
    }
    /* Stream channel count follows the layout; the marker goes in the streamed channel that maps to
     * the cabled port pair. In 2T2R both channels stream, so --chan picks the interleave slot; in
     * 1T1R only one channel streams (slot 0) and the second physical port pair is selected inside
     * the RFIC (same convention as loopback_selftest). */
    g_nch   = layout2 ? 2 : 1;
    g_mchan = (layout2 && chan == 2) ? 1 : 0;
    if (!layout2 && chan == 2)
    ...
    cfg.tx_att = tx_att; cfg.rx_gain1 = rx_gain; cfg.rx_gain2 = rx_gain; cfg.program_rx_gains = true;
    cfg.channel_layout = layout2 ? M2SDR_CHANNEL_LAYOUT_2T2R : M2SDR_CHANNEL_LAYOUT_1T1R;
    cfg.clock_source = M2SDR_CLOCK_SOURCE_INTERNAL;
    if (m2sdr_apply_config(dev, &cfg) != 0) { fprintf(stderr, "apply_config failed\n"); return 1; }

    /* SC16 only: the marker synthesis and RX edge/peak search below are written against int16
     * samples. An sc8 mode would need those three helpers reworked for int8 packing -- worth doing
     * if the top rates must be measured over a link too narrow for sc16. */
    ...
    /* A timed marker must traverse the TX DMA ring before it can air: the LOOP-mode reader walks
     * the ring in order, so a marker submitted now airs ~one ring-depth later. If the requested
     * lead is shorter than that pipeline latency, the marker reaches the gate after its air-time
     * and is missed (its header ts reads as untimed). Floor the lead to the ring latency (+2x
     * margin); the floor therefore scales with the ring depth and the sample rate (e.g. ~8.5 ms at
     * 30.72 MSPS, ~2.1 ms at 122.88 MSPS for a 256-buffer ring). */
    double   ring_latency_s = (double)DMA_BUFFER_COUNT * ((double)g_pairs / (double)g_nch) / fs;
    uint64_t min_lead_ns    = (uint64_t)(ring_latency_s * 5.0 * 1e9);
    uint64_t lead_ns        = (uint64_t)(lead_us * 1000.0);
    if (lead_ns < min_lead_ns) {
        printf("note: requested lead %.1f ms < TX DMA ring latency; raising to %.1f ms "
               "(%d buffers x %u samp / %d ch / %.2f MSPS = %.1f ms one-way)\n",
               lead_us / 1000.0, min_lead_ns / 1e6, DMA_BUFFER_COUNT, g_pairs, g_nch,
               fs / 1e6, ring_latency_s * 1e3);
        lead_ns = min_lead_ns;
    }
    ...
    if (nD >= 2) {
        ...
        /* Determinism threshold scales with the RX sample period: the reported spread is dominated
         * by RX edge-detection quantization (+/- one sample), which is 8.1 ns @122.88 but 32.6 ns
         * @30.72. Allow up to ~one sample period (min 15 ns) so a coarser rate is not failed for
         * quantization noise the gate does not produce. */
        double sample_ns = 1e9 / fs;
        double det_thr   = sample_ns > 15.0 ? sample_ns : 15.0;
        int det = sd < det_thr;
        printf("CHECK timed.deterministic  %s  std %.1f ns (< %.1f ns = 1 RX sample; spread %.0f ns = RX edge quantization)\n",
               det ? "PASS" : "FAIL", sd, det_thr, mx - mn);
