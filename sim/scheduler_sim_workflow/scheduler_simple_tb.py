...
def stimulus(dut):
    ...
    yield from drive_packet(dut, 
                            ts_when_due = frames_per_packet , 
                            n_words = frames_per_packet * 2)
    time += frames_per_packet * 2
    yield from wait(dut, 
                    wait_cycles = frames_per_packet * 2)
    time += frames_per_packet * 2

    yield from drive_packet(dut, 
                            ts_when_due = time + frames_per_packet * 1, 
                            n_words = frames_per_packet * 1)
    yield from wait(dut, wait_cycles = frames_per_packet * 4)
